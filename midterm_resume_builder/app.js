...
form.addEventListener('submit', function (e) {
  ...
  ///=======================================
  const companyNames = form.elements.companyName
  const companyPositions = form.elements.companyPosition
  const companyStartDates = form.elements.companyStartDate
  const companyEndDates = form.elements.companyEndDate
  const companyDescs = form.elements.companyDesc


  if (companyNames.length > 1) {
    document.querySelector('.workExperience').innerHTML = '';

    for (let i = 0; i < companyNames.length; i++) {
      const workTemplate = `<span id="company-name">${companyNames[i].value}</span> <span id="position-name">${companyPositions[i].value}</span> <span
      id="position-start-date-span">${companyStartDates[i].value}</span> - <span id="position-end-date-span">${companyEndDates[i].value}</span>
      <div id="description-span">${companyDescs[i].value}</div>`

      const div = document.createElement('div')
      div.innerHTML = workTemplate;
      div.style.marginBottom = '20px'
      div.style.borderBottom = '2px solid rgb(28, 28, 177)'
      div.style.paddingBottom = '20px'
      console.log(div)
      document.querySelector('.workExperience').appendChild(div)

    }
  } else {

    document.getElementById("company-name-span").innerText = companyNameVal
    document.getElementById("position-name-span").innerText = positionVal
...
document.querySelector('.fa-plus').addEventListener('click', function () {
  ...
  var cln = work.cloneNode(true);
  document.getElementById('work-container').appendChild(cln)
})